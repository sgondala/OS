...
pair<int,int> writeMultipleBlocks(int startBlockNum, int startByteNum, string data, 
	...
	fseekLock.unlock();
	blockLock[lockingBlock].unlock();
	if(entered) temporaryByte = stringRemaining.length();
	if(!entered) temporaryByte += stringRemaining.length();
...
pair<string,pair<int,int> > readMultipleBlocks(int startBlockNum, int startByteNum, 
	...
	if(startByteNum==maxDataByte+1){
		...
		if(temp==0){
			printf("Process %i Unable to read \n", pid);
			return make_pair(data,make_pair(startBlockNum, startByteNum));
		}
	}
	...
	while(remaining > maxDataByte - temporaryByte + 1){
		...
		printf("read: disk block %i read \n", lockingBlock);
		
		if(temp==0){
			printf("Process %i - Read exceeeded the size \n", pid);
			blockLock[lockingBlock].unlock();
			return make_pair(data,make_pair(startBlockNum, startByteNum)); 
...
void *threadFunction(void *InData){//CHANGE
	...
	while(getline(ifs,line)){
		...
		if(line.substr(0,4)=="read"){
			...
			int fileLocation = 	fileStartingBlock(fileName, &dir);
			if(fileLocation==-1){
				printf("Cannot read file %s - Does not exist in directory \n", fileName.c_str());
				continue;
			}
			else{
				...
				if(currentBlock != -1){ // File already opened
					...
					lastFileIndex = index;
					printf("Read val is %s \n", returnVal.first.c_str());
				}
					...
					lastFileIndex = fcbVector.size() - 1;
					printf("File opened and read. Read val is %s \n", returnVal.first.c_str());
				}	
			}
			continue;
		}
